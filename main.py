        ...
        for i,h in enumerate(headers):
            for p in patterns:
                if re.search(p, h, flags=re.I):
                    return i
        return -1
    idx_vessel = find([r"vessel", r"vessel name"])
    idx_voyage = find([r"voy", r"voyage"])
    idx_service= find([r"service"])
    idx_term   = find([r"terminal"])
    idx_etd    = find([r"etd"])
    idx_eta    = find([r"eta"])
    idx_doc    = find([r"doc.*off|instruction"])
    idx_desp   = find([r"customs|despacho"])
    out = []
    for r in data[1:]:
        out.append({
            "service": (r[idx_service] if idx_service>=0 else "").strip(),
            "vessel":  (r[idx_vessel]  if idx_vessel >=0 else "").strip(),
            "voyage":  (r[idx_voyage]  if idx_voyage >=0 else "").strip(),
            "terminal":(r[idx_term]    if idx_term   >=0 else "").strip(),
            "ETD_local": r[idx_etd] if idx_etd>=0 else "",
            "ETA_local": r[idx_eta] if idx_eta>=0 else "",
            "DOC_cutoff": r[idx_doc] if idx_doc>=0 else "",
            "DESPACHO_cutoff": r[idx_desp] if idx_desp>=0 else "",
        })
    return out
...
async def unified(pol: str = "Barcelona"):
    ...
    rows += pull_maersk()
    rows += pull_hapag()
    # MSC: mejor esfuerzo (puede venir vacío si cambió el sitio)
    try:
        rows += await pull_msc_async()
    except Exception:
        pass

    # Normaliza estatus (buffers: DOC 36h, DESPACHO 24h), Hapag: DESPACHO=ETA-24h con regla de viernes si no viene
    out = []
    for r in rows:
        doc = to_dt(r.get("DOC_cutoff",""))
        desa = r.get("DESPACHO_cutoff","")
        desp = to_dt(desa) if desa else None
